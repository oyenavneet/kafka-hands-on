...
public class UnorderedMessageProcessorConfig {
    ...
    private static final Logger logger = LoggerFactory.getLogger(UnorderedMessageProcessorConfig.class);
...
//                .map(deliveryService::toDelivery) // it iterate over and send one by one
                .gather(Gatherers.mapConcurrent(MAX_CONCURRENCY, deliveryService::toDelivery)) // it processes concurrent
                .map(this::toMessage)
                .toList();
    }
